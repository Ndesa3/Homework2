...
dt1 = datetime.now()
cond_or = "or" in query
cond_not_or = "or" not in query
cond_and = "and" in query
cond_not_and = "and" not in query
if (cond_or) and (cond_not_and):    #if "and" is present, "and" operation takes over from elif statement
    ...
    dt2= datetime.now()
    print("Execution time:", dt2.microsecond-dt1.microsecond)        
elif(cond_not_and) and (cond_not_or):
    for i,quote in enumerate(data_list):
        if (query[0] in quote) and (query[1] in quote):
            print("Found:", i, "...", quote[:500],"...")
    dt2= datetime.now()
